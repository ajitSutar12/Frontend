...
export class AppComponent implements OnInit {
    ...
    this._hotkeysService.add(new Hotkey(['f2', 'f5', 'f6', 'f7', 'f8', 'f9', 'f10', 'f11', 'f12',
      ...
      (event: KeyboardEvent, combo: string): boolean => {
        ...
        // }
          ...
          this.router.navigate([]).then(result => { window.open(this.site + '/accountenq', '', "width=1200,height=800"); });
        }
        else if (combo === 'ctrl+f8' || combo === 'ctrl+fn+f8') {
          this.router.navigate([]).then(result => { window.open(this.site + '/custview', '', "width=1200,height=800"); });
        }
        else if (combo === 'ctrl+fn+f9' || combo === 'f9') {
          this.router.navigate([]).then(result => { window.open(this.site + '/ledgers', '', "width=1200,height=800"); });
        }
        else if (combo === 'ctrl+fn+f11' || combo === 'f11') {
          this.router.navigate([]).then(result => { window.open(this.site + '/managerview', '', "width=1200,height=800"); });
        }
        else if (combo === 'ctrl+fn+f12' || combo === 'f12') {
          this.router.navigate([]).then(result => { window.open(this.site + '/centralpassing', '', "width=1200,height=800"); });
        }
        else if (combo === 'ctrl+f3' || combo === 'ctrl+f3') {
          this.router.navigate([]).then(result => { window.open(this.site + '/voucherview', '', "width=1200,height=800"); });
        }
        else if (combo === 'shift+f5') {
          this.router.navigate([]).then(result => { window.open(this.site + '/batchVouchers', '', "width=1200,height=800"); });
        }
        return false;
  ...
  //     (event: KeyboardEvent, combo: string): boolean => {
  //       if (combo === 'ctrl+fn+f2' || combo === 'f2') {
  //         this.router.navigate([]).then(result => { window.open(this.site + '/utility/calculator', '', "width=1200,height=800"); });
  //       }
  //       else if (combo === 'ctrl+fn+f3' || combo === 'f3') {
