...
export class SchemeCodeDropdownService {
    ...
    public getSchemeCodeList(scheme) {
        ...
        return this.http.get<any>(this.url + '/scheme-parameters/' + scheme)
            .pipe(map(ele => {
                ele.forEach(element => {
                    let obj = { label: element.S_APPL + ' ' + element.S_NAME, value: element.id, id: element.S_APPL, name: element.S_ACNOTYPE, dividend: element.DIVIDEND_PERCENTAGE, divMethod: element.INTEREST_RULE, isAddBonus: element.IS_ADD_BONUS_IN_DIVIDEND, INT_ROUND_OFF: element.INT_ROUND_OFF, IS_WEEKLY_REPAY: element.IS_WEEKLY_REPAY, IS_RECURRING_TYPE: element.IS_RECURRING_TYPE };
