...
export class BalanceUpdationService {
    ...
    postData(data1: any): Observable<any> {
        return this.http.post(this.url + '/balance-updation/insert', data1).pipe(map((res) => res),
            catchError((error) => {
    ...
    dailyTableInsert(data1: any): Observable<any> {
        return this.http.post(this.url + '/balance-updation/dailyInsert', data1).pipe(map((res) => res),
            catchError((error) => {
                let errorMessage = 'Please add valid length';
                console.log(error)
                // window.alert(errorMessage);
