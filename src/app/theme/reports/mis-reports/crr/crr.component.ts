...
Types = [
 ...
 view(event) {
  ...
  if (this.angForm.controls['RADIO'].value=="detail" && this.angForm.valid) {
     ...
     this.iframe1url = this.report_url+"examples/CRR.php?stadate='" + stadate + "'&edate='" + edate + "'&branched='" + branched + "'&detail='" + summary +  "'&startscheme='" + startscheme + "'&bankName='" + bankName + "'&branchName='"+this.branchName+"'";
     console.log(this.iframe1url);
     this.iframe1url = this.sanitizer.bypassSecurityTrustResourceUrl(this.iframe1url);
   }
   else {
